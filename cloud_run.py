...
def apply_curation(alljobs):
    ...
    tready = cur.get("tailor_ready", {})   # roles Claude has triaged + picked a base CV for (badge 🎯)
    have = set()
    for r in alljobs:
        ...
        if tr:
            ...
            if tr.get("region") and r.get("region") in ("", "Unknown"):
...
def main():
    ...
    counts = {}; bucket = []; qa_bucket = []; giant_bucket = []
    ...
    # Giant-bucket: every giant-company role (all levels), non-senior first so the
    # actionable ones stay on top when the cap trims. Reviewed manually, like the others.
    _gpri = {"open": 0, "student": 1, "senior": 2}
    giant_bucket.sort(key=lambda b: _gpri.get(b["reason"].split(":")[-1], 3))
    J.atomic_write(GIANT_JSON, json.dumps(
        {"generated": J.TODAY, "count": len(giant_bucket), "roles": giant_bucket[:BUCKET_CAP]}, ensure_ascii=False))
