...
(async () => {
  ...
  assert.equal(await page.locator('#jobStatus').inputValue(),'Applied');
  // Skipping hides the role and persists; undo preserves its application stage and notes.
  await page.locator(`[data-skip="${quickId}"]`).click();
  assert.equal(await page.locator(`[data-job="${quickId}"]`).count(),0);
  await page.reload();await page.waitForSelector('.card');
  assert.equal(await page.locator(`[data-job="${quickId}"]`).count(),0);
  await page.locator('[data-view="skipped"]').click();
